...
class RowMatrixOperations {
    ...
    static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB)
    {
        ...
        else
        {
            ...
            auto ret = std::make_unique<RowMatrix<T>>(row, col);
            for (int r = 0; r < row; ++r)
            {
                for (int c = 0; c < col; ++c)
                {
                    ret->SetElement(r, c, matrixA->GetElement(r,c) + matrixB->GetElement(r,c));
                }
            }
            return ret;
        }
        ...
        else
        {
            ...
            for (int r = 0; r < row; ++r)
            {
                for (int c = 0; c < col; ++c)
                {
                    ...
                    for (int k = 0; k < mid; ++k)
                    {
                        elem += matrixA->GetElement(r,k) * matrixB->GetElement(k,c);
                    }
                    ret->SetElement(r, c, elem);
                }
            }
            return ret;
        }
    ...
    static void Print(const RowMatrix<T> &matrix)
    {
        ...
        for (int r = 0; r < matrix.GetRowCount(); ++r)
        {
            ...
            {
                printf("%2d ", matrix.GetElement(r,c));
            }
            printf("\n");
        }
        printf("\n");
